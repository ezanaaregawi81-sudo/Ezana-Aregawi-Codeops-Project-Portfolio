...
export default function Login() {
  ...
  if (localStorage.getItem("addis-eatery-signed-in") === "true") return <Navigate to="/checkout" replace />;
  const destination = location.state?.from?.pathname || "/checkout";
  const submit = (event) => { event.preventDefault(); localStorage.setItem("addis-eatery-signed-in", "true"); navigate(destination, { replace: true }); };
