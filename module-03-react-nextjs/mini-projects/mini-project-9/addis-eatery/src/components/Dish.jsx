...
export default function Dish({ dishes, onAdd, onQuickView }) {
      ...
      {dishes.map((dish) => (
        <li key={dish.id} className="dish-card">
          ...
          <div className="dish-actions">
            <button type="button" className="secondary-btn" onClick={() => onQuickView(dish)}>
              Quick view
            </button>
            <button type="button" onClick={() => onAdd(dish)}>Add</button>
          </div>
        </li>
      ))}
    </ul>
