...
export default function CartPage({ searchParams }) {
  ...
  const subtotal = useMemo(
    () => basketItems.reduce((acc, item) => acc + Number(item.price || 0) * Number(item.qty || 0), 0),
    [basketItems],
  );
  const deliveryFee = basketItems.length > 0 ? 100 : 0;
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '2rem' }}>
        <div className="ht-panel" style={{ height: 'fit-content', padding: '1.75rem' }}>
          <h3 style={{ fontSize: '1.3rem', fontWeight: '700', marginBottom: '1.25rem', borderBottom: '1px solid var(--ht-border)', paddingBottom: '0.75rem' }}>
            Order Summary
          </h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem', fontSize: '0.95rem' }}>
            ...
            <div style={{ borderTop: '1px dashed var(--ht-border)', paddingTop: '0.75rem', marginTop: '0.5rem', display: 'flex', justifyContent: 'space-between', fontWeight: '800', fontSize: '1.2rem' }}>
              <span>Total</span>
              <span style={{ color: 'var(--ht-primary-strong)' }}>{total} ETB</span>
            </div>
          </div>

          <Link href={basketItems.length > 0 ? '/checkout' : '/menu'} className="ht-btn ht-btn--primary" style={{ width: '100%', padding: '0.9rem' }} id="cart-proceed-checkout-btn">
            {basketItems.length > 0 ? 'Proceed to Checkout →' : 'Choose Dishes First'}
          </Link>
        </div>
            ...
            basketItems.map((item) => (
              <div key={item.id} className="ht-panel" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem', gap: '1rem', flexWrap: 'wrap' }}>
                ...
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: '1.1rem', fontWeight: '700', color: 'var(--ht-primary-strong)' }}>
