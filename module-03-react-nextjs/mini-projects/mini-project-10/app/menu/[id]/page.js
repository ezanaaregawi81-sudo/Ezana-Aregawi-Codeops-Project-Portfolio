...
// Generate static params for known dish IDs
...
export default async function DishDetailPage({ params }) {
  // Requirement 2: Extract dish ID directly from component params (await params)
  const { id } = await params;
  const dish = getMenuItemById(id);

  // Requirement 5: Trigger custom 404 page if dish is not found
  if (!dish) {
    notFound();
  }
    ...
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      ...
      <div className="ht-panel" style={{ padding: '2.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center', flexWrap: 'wrap', marginBottom: '2rem' }}>
          <div
            className="ht-dish-row__media"
            style={{ width: '160px', height: '160px', fontSize: '5rem', flexShrink: 0 }}
          >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
              <span className="ht-tag ht-tag--primary">{dish.category}</span>
              <span className={`ht-tag ${dish.isFasting ? 'ht-tag--fasting' : 'ht-tag--primary'}`}>
                {dish.isFasting ? 'Fasting 🌱' : 'Traditional 🥩'}
              </span>
              <span className="ht-tag ht-tag--spice">{dish.spiceLevel}</span>
            </div>
            ...
            <div style={{ fontSize: '1.8rem', fontWeight: '800', color: 'var(--ht-primary-strong)' }}>
              {dish.priceFormatted}
            </div>
          </div>
        </div>
